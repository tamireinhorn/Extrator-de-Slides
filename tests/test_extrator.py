...
class TestInput:
    def test_negative_input(self, monkeypatch):
        ...
        with pytest.raises(
            ValueError,
            match="Valor inválido. Segundos são um número inteiro maior que 0.",
        ):
            receive_input()
...
class TestOpenVideoFile:
    def test_if_not_video(self, monkeypatch):
        ...
        with pytest.raises(
            ValueError,
            match="O programa apenas suporta arquivos de vídeo. Escolha um arquivo de vídeo.",
        ):
            choose_video()
...
class TestVideoLengthFunction:
    ...
    def test_no_possible_iterations(self, monkeypatch, file_dialog_mock):
        ...
        with pytest.raises(
            ValueError,
            match=f"Você escolheu {seconds} segundos para um slide ser relevante, mas o vídeo dura só {video_length} segundos",
        ):
...
class TestImageComparison:
    def test_can_distinguish_images(self, file_dialog_mock):
        ...
        cap.set(
            0, video_length * 1000
        )  # To get last frame, video_length in seconds converted to MS.
        _, imageB = cap.read()
    ...
    def test_image_comparison_invalid_threshold(self, file_dialog_mock):
        ...
        with pytest.raises(
            ValueError,
            match="A tolerância para similaridade tem que ser um número positivo entre 0 e 1.",
        ):
...
class TestFolderCreation:
    ...
    def test_create_new_folder_with_absolute_path(
        self, file_dialog_mock, folder_cleanup
    ):
